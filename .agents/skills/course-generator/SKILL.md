...
Copy these into the course `images/` folder (in this authoring template they already ship under `course/images/`; otherwise copy from the HTML Slides Viewer repo `images/` directory, or any course that already has them). **Do not regenerate** them; reuse the same filenames and wire them on the matching slides:
